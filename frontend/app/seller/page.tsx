      ...
      {selectedProduct && (
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4" onClick={handleCloseModal}>
          <div className="bg-white rounded-lg max-w-2xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            {/* Product Details */}
            <div className="p-6">
              <h2 className="text-2xl font-bold text-gray-900 mb-4">{selectedProduct.product_detail}</h2>

              <div className="grid grid-cols-2 gap-4 mb-6">
                <div>
                  <p className="text-gray-600 text-sm">Asking Price</p>
                  <p className="text-3xl font-bold text-blue-600">${selectedProduct.asking_price}</p>
                </div>
                <div>
                  <p className="text-gray-600 text-sm">Min Selling Price</p>
                  <p className="text-xl font-semibold text-gray-900">${selectedProduct.min_selling_price}</p>
                </div>
                <div>
                  <p className="text-gray-600 text-sm">Condition</p>
                  <span className={`inline-block px-3 py-1 rounded font-semibold text-sm ${getConditionColor(selectedProduct.condition)}`}>
                    {selectedProduct.condition}
                  </span>
                </div>
                <div>
                  <p className="text-gray-600 text-sm">Category</p>
                  <p className="text-lg font-semibold text-gray-900 capitalize">{selectedProduct.category.replace("-", " ")}</p>
                </div>
              </div>

              <div className="mb-6 pb-6 border-b border-gray-200">
                <p className="text-gray-600 text-sm mb-2">Location</p>
                <p className="text-lg font-semibold text-gray-900">{selectedProduct.location}, {selectedProduct.zip_code}</p>
              </div>

              <div className="mb-6">
                <p className="text-gray-600 text-sm mb-1">Seller</p>
                <p className="text-gray-900 font-mono text-sm">{selectedProduct.seller_id}</p>
              </div>
